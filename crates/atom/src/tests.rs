#[cfg(test)]
    ...
    use crate::runtime::{NucleusRuntime, AppState};
    use std::sync::Arc;
    ...
    async fn test_runtime_benchmark_route() {
        ...
        routes.insert("plaintext".to_string(), axum::body::Bytes::from("Hello World"));
        ...
        let res = app
            .oneshot(Request::builder().uri("/plaintext").body(Body::empty()).unwrap())
            .await
            .unwrap();
    ...
    async fn test_runtime_mime_types() {
        ...
        let mut routes = AHashMap::new();
        routes.insert("styles.css".to_string(), axum::body::Bytes::from("body { color: red; }"));
