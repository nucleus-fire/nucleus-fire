    ...
    pub fn parse(input: &str) -> Self {
        ...
        if let Some(stripped) = trimmed.strip_prefix('.') {
            ...
            match parts.first().map(|s| s.to_lowercase()).as_deref() {
                ...
                Some("describe") | Some("desc") | Some("d") => {
                    parts.get(1).map_or(Command::Help, |t| Command::Describe((*t).to_string()))
                }
...
impl Console {
    ...
    pub fn new(config: ConsoleConfig) -> Result<Self> {
        ...
        Ok(Self { config, conn: None, editor })
    }
    ...
    fn print_banner(&self) {
        ...
        println!("  \x1b[1;36m║\x1b[0m   Interactive Database REPL              \x1b[1;36m║\x1b[0m");
    ...
    fn list_tables(&self) -> Result<()> {
        ...
        let tables: Vec<String> = stmt.query_map([], |row: &rusqlite::Row| row.get(0))
            .map_err(|e| miette!("Query error: {}", e))?
            .filter_map(|r| r.ok())
            .collect();
    ...
    fn show_info(&self) -> Result<()> {
        ...
        let version: String = conn.query_row("SELECT sqlite_version()", [], |row: &rusqlite::Row| row.get(0))
            .map_err(|e| miette!("Query error: {}", e))?;
...
mod tests {
    ...
    fn test_command_parse_describe() {
        assert_eq!(Command::parse(".describe users"), Command::Describe("users".to_string()));
    }
    
    #[test]
    fn test_command_parse_sql() {
        assert_eq!(Command::parse("SELECT * FROM users"), Command::Sql("SELECT * FROM users".to_string()));
    }
    ...
    fn test_extract_database_url() {
        ...
        assert_eq!(extract_database_url(content), Some("sqlite:./data.db".to_string()));
    }
}
