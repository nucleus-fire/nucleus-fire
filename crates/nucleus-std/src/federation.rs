...
impl SourceQuery {
    ...
    pub async fn fetch(self) -> Result<Vec<ContentItem>> {
        match self.source.as_str() {
            ...
            _ => Err(NucleusError::InternalError(format!("Unknown source: {}", self.source))),
        }
    }
}
...
impl DirectusAdapter {
    ...
    pub async fn fetch_collection(collection: Option<&str>) -> Result<Vec<ContentItem>> {
        let collection = collection.ok_or_else(|| {
            NucleusError::ValidationError("Collection name required".to_string())
        })?;
    ...
    async fn http_get(url: &str, token: Option<&str>) -> Result<Vec<ContentItem>> {
        ...
        if !res.status().is_success() {
            return Err(NucleusError::InternalError(format!("HTTP error: {}", res.status())));
        }
        ...
        Ok(items.iter().enumerate().map(|(i, item)| ContentItem {
            id: item.get("id").and_then(|v| v.as_str()).unwrap_or(&format!("{}", i)).to_string(),
            source: "directus".to_string(),
            collection: "items".to_string(),
            data: item.clone(),
            created_at: item.get("date_created").and_then(|v| v.as_str()).map(String::from),
            updated_at: item.get("date_updated").and_then(|v| v.as_str()).map(String::from),
        }).collect())
    }
...
impl SanityAdapter {
    ...
    pub async fn fetch_groq(groq: Option<&str>) -> Result<Vec<ContentItem>> {
        ...
        let dataset = std::env::var("SANITY_DATASET")
    ...
    fn to_groq(query: &FederationQuery) -> String {
        ...
        if !query.filters.is_empty() {
            let filters: Vec<String> = query.filters
                ...
                .map(|(k, v)| format!("{} == {}", k, v))
                .collect();
            groq = format!("*[_type == '{}' && {}]", query.collection, filters.join(" && "));
        }
    ...
    async fn http_get(url: &str, _token: Option<&str>) -> Result<Vec<ContentItem>> {
        ...
        let items = body.get("result").and_then(|d| d.as_array())
            .ok_or_else(|| NucleusError::InternalError("Invalid Sanity response".to_string()))?;
        
        Ok(items.iter().enumerate().map(|(i, item)| ContentItem {
            id: item.get("_id").and_then(|v| v.as_str()).unwrap_or(&format!("{}", i)).to_string(),
            source: "sanity".to_string(),
            collection: item.get("_type").and_then(|v| v.as_str()).unwrap_or("document").to_string(),
            data: item.clone(),
            created_at: item.get("_createdAt").and_then(|v| v.as_str()).map(String::from),
            updated_at: item.get("_updatedAt").and_then(|v| v.as_str()).map(String::from),
        }).collect())
    }
...
impl StrapiAdapter {
    ...
    pub async fn query(query: &FederationQuery) -> Result<Vec<ContentItem>> {
        let base_url = std::env::var("STRAPI_URL")
    ...
    async fn http_get(url: &str, token: Option<&str>) -> Result<Vec<ContentItem>> {
        ...
        Ok(items.iter().enumerate().map(|(i, item)| ContentItem {
            id: item.get("id").map(|v| v.to_string()).unwrap_or_else(|| format!("{}", i)),
            source: "strapi".to_string(),
            collection: "items".to_string(),
            data: item.get("attributes").cloned().unwrap_or(item.clone()),
            created_at: item.get("attributes").and_then(|a| a.get("createdAt")).and_then(|v| v.as_str()).map(String::from),
            updated_at: item.get("attributes").and_then(|a| a.get("updatedAt")).and_then(|v| v.as_str()).map(String::from),
        }).collect())
    }
    ...
    pub fn set(&self, key: &str, data: Vec<ContentItem>) {
        if let Ok(mut entries) = self.entries.write() {
            entries.insert(key.to_string(), CacheEntry {
                data,
                expires_at: std::time::Instant::now() + std::time::Duration::from_secs(self.ttl_seconds),
            });
        }
    }
...
mod tests {
    ...
    async fn test_directus_mock_query() {
        let items = DirectusAdapter::query(&FederationQuery::new("articles")).await.unwrap();
    ...
    async fn test_sanity_mock_query() {
        let items = SanityAdapter::query(&FederationQuery::new("article")).await.unwrap();
    ...
    async fn test_strapi_mock_query() {
        let items = StrapiAdapter::query(&FederationQuery::new("articles")).await.unwrap();
    ...
    fn test_cms_type_deserialization() {
        let config: SourceConfig = serde_json::from_str(r#"{
            "name": "my-cms",
            "type": "directus",
