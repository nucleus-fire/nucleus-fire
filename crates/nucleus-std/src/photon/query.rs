...
impl<'a> Builder<'a> {
    ...
    pub fn order_by(mut self, column: &str, direction: &str) -> Self {
        self.order_by
            .push((column.to_string(), direction.to_uppercase()));
        self
    }
    ...
    {
        self.values
            .push((column.to_string(), QueryValue::from(value)));
        self
    }
        ...
        T: for<'r> FromRow<'r, sqlx::sqlite::SqliteRow> + Send + Unpin,
    {
        ...
        Err(sqlx::Error::Configuration(
            "Unsupported database type for this query".into(),
        ))
    }
...
where
    F: for<'c> FnOnce(
        &'c mut sqlx::Transaction<'_, sqlx::MySql>,
    ) -> Pin<Box<dyn Future<Output = Result<T, sqlx::Error>> + Send + 'c>>,
{
    let pool = db();
...
#[deprecated(
    since = "0.2.0",
    note = "Use transaction_sqlite, transaction_postgres, or transaction_mysql"
)]
...
mod tests {
    ...
    #[test]
    fn test_delete_sql() {
        let builder = Builder::new("users").delete().r#where("id", 1i64);
    ...
    fn test_limit_offset() {
        let builder = Builder::new("users").limit(10).offset(20);
    ...
    #[test]
    fn test_in_operator() {
        let builder = Builder::new("users").filter_op("id", Op::In, "1,2,3");
    ...
    fn test_not_in_operator() {
        let builder = Builder::new("users").filter_op("status", Op::NotIn, "banned,suspended");
    ...
    #[test]
    fn test_is_not_null() {
        let builder = Builder::new("users").filter_op("email_verified", Op::IsNotNull, 0i64);
    ...
    fn test_like_operator() {
        let builder = Builder::new("users").filter_op("name", Op::Like, "%john%");
    ...
    fn test_select_specific_columns() {
        let builder = Builder::new("users").select(&["id", "name", "email"]);
    ...
    #[test]
    fn test_right_join() {
        let builder = Builder::new("posts").right_join("users", "posts.user_id", "users.id");
