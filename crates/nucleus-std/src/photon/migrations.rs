...
async fn ensure_migrations_table() -> Result<(), MigrationError> {
    ...
    match pool {
        DatabasePool::Sqlite(p) => { sqlx::query(sql).execute(p).await?; }
        DatabasePool::Postgres(p) => { sqlx::query(sql).execute(p).await?; }
        DatabasePool::MySql(p) => { sqlx::query(sql).execute(p).await?; }
    }
...
pub async fn run_migrations(dir: &str) -> Result<Vec<String>, MigrationError> {
    ...
    for (name, path) in migrations {
        ...
        // Execute
        match pool {
            DatabasePool::Sqlite(p) => { sqlx::query(&up_sql).execute(p).await?; }
            DatabasePool::Postgres(p) => { sqlx::query(&up_sql).execute(p).await?; }
            DatabasePool::MySql(p) => { sqlx::query(&up_sql).execute(p).await?; }
        }
        
        // Record migration
        let record_sql = "INSERT INTO _migrations (name) VALUES (?)";
        match pool {
            DatabasePool::Sqlite(p) => { sqlx::query(record_sql).bind(&name).execute(p).await?; }
            DatabasePool::Postgres(p) => { 
                sqlx::query("INSERT INTO _migrations (name) VALUES ($1)")
                    .bind(&name).execute(p).await?; 
            }
            DatabasePool::MySql(p) => { sqlx::query(record_sql).bind(&name).execute(p).await?; }
        }
...
pub async fn rollback(n: usize) -> Result<Vec<String>, MigrationError> {
    ...
    for (name, _applied_at) in to_rollback {
        ...
        match pool {
            DatabasePool::Sqlite(p) => { sqlx::query(delete_sql).bind(&name).execute(p).await?; }
            DatabasePool::Postgres(p) => { 
                sqlx::query("DELETE FROM _migrations WHERE name = $1")
                    .bind(&name).execute(p).await?; 
            }
            DatabasePool::MySql(p) => { sqlx::query(delete_sql).bind(&name).execute(p).await?; }
        }
...
async fn get_applied_migrations_map() -> Result<std::collections::HashMap<String, DateTime<Utc>>, MigrationError> {
    let applied = get_applied_migrations_ordered().await?;
