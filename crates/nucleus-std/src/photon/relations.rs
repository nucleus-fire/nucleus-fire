...
#[deprecated(
    since = "0.2.0",
    note = "Use load_has_many_sqlite, load_has_many_postgres, or load_has_many_mysql"
)]
...
#[deprecated(
    since = "0.2.0",
    note = "Use load_belongs_to_sqlite, load_belongs_to_postgres, or load_belongs_to_mysql"
)]
...
mod tests {
    ...
    fn test_multiple_parents_ids() {
        let users = vec![MockUser { id: 1 }, MockUser { id: 2 }, MockUser { id: 3 }];

        let ids: Vec<i64> = users
            .iter()
            .map(|u| <MockUser as HasMany<MockPost>>::get_id(u))
            .collect();
