...
impl<B: CacheBackend> UnifiedCache<B> {
    ...
    pub async fn set_with_ttl<T: Serialize>(
        ...
        let full_key = self.make_key(key);
        let bytes =
            serde_json::to_vec(value).map_err(|e| RedisCacheError::Serialization(e.to_string()))?;
...
mod tests {
    ...
    async fn test_memory_backend_set_get() {
        ...
        backend
            .set_bytes("key", b"value".to_vec(), 300)
            .await
            .unwrap();
    ...
    async fn test_memory_backend_delete_pattern_prefix() {
        ...
        backend
            .set_bytes("user:1:name", b"alice".to_vec(), 300)
            .await
            .unwrap();
        backend
            .set_bytes("user:1:email", b"alice@test.com".to_vec(), 300)
            .await
            .unwrap();
        backend
            .set_bytes("user:2:name", b"bob".to_vec(), 300)
            .await
            .unwrap();
    ...
    async fn test_memory_backend_delete_pattern_suffix() {
        ...
        backend
            .set_bytes("user:1:cache", b"1".to_vec(), 300)
            .await
            .unwrap();
        backend
            .set_bytes("user:2:cache", b"2".to_vec(), 300)
            .await
            .unwrap();
        backend
            .set_bytes("user:1:data", b"3".to_vec(), 300)
            .await
            .unwrap();
    ...
    async fn test_memory_backend_ttl() {
        ...
        // Key without TTL
        backend
            .set_bytes("eternal", b"value".to_vec(), 0)
            .await
            .unwrap();
    ...
    async fn test_memory_backend_expiration() {
        ...
        backend
            .set_bytes("key", b"value".to_vec(), 1)
            .await
            .unwrap();
    ...
    async fn test_unified_cache_delete_pattern() {
        ...
        cache
            .set("user:1:name", &"alice".to_string())
            .await
            .unwrap();
        cache
            .set("user:1:email", &"alice@test.com".to_string())
            .await
            .unwrap();
    ...
    async fn test_unicode_key() {
        ...
        assert_eq!(
            cache.get::<String>("用户:123").await.unwrap(),
            Some("chinese".to_string())
        );
        assert_eq!(
            cache.get::<String>("🔑:emoji").await.unwrap(),
