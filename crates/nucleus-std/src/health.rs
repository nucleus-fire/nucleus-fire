...
type BoxedCheck = Box<dyn Fn() -> Pin<Box<dyn Future<Output = HealthStatus> + Send>> + Send + Sync>;
    ...
    pub async fn check_all(&self) -> HealthReport {
        ...
        for check in checks.iter() {
            ...
            let status = match tokio::time::timeout(check.timeout, (check.check)()).await {
                Ok(s) => s,
                Err(_) => {
                    HealthStatus::Unknown(format!("Check timed out after {:?}", check.timeout))
                }
            };
...
mod tests {
    ...
    async fn test_checker_healthy_check() {
        let checker = HealthChecker::new();
        checker
            .add_check("test", || async { HealthStatus::Healthy })
            .await;
    ...
    async fn test_checker_multiple_checks() {
        ...
        checker
            ...
            .await;
        checker
            .add_check("api", || async { HealthStatus::Healthy })
            .await;
    ...
    async fn test_checker_optional_check_failure() {
        let checker = HealthChecker::new();
        checker
            .add_check("critical", || async { HealthStatus::Healthy })
            .await;
        checker
            .add_optional_check("optional", || async {
    ...
    fn test_component_check_creation() {
        let check =
            ComponentCheck::new("database", HealthStatus::Healthy, Duration::from_millis(50));
    ...
    async fn test_concurrent_checks() {
        ...
        for i in 0..5 {
            ...
            checker
                .add_check(&name, move || async move {
    ...
    async fn test_mixed_critical_optional() {
        ...
        // Critical healthy
        checker
            .add_check("critical1", || async { HealthStatus::Healthy })
            .await;

        // Critical unhealthy
        checker
            .add_check("critical2", || async {
                HealthStatus::Unhealthy("Down".into())
            })
            .await;
