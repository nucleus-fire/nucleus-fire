...
impl Browser {
    ...
    pub fn goto(&self, url: &str) -> Result<String, BrowserError> {
        ...
        let remote_object = tab.evaluate("document.documentElement.outerHTML", false)
    ...
    pub fn get_title(&self, url: &str) -> Result<String, BrowserError> {
        ...
        let remote_object = tab.evaluate("document.title", false)
            .map_err(|e| BrowserError::EvalFailed(e.to_string()))?;
        
        Ok(remote_object.value
...
mod tests {
    ...
    fn test_browser_error_display() {
        ...
        assert_eq!(err.to_string(), "JavaScript evaluation failed: syntax error");
