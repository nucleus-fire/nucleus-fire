...
impl StreamHub {
    ...
    pub async fn unregister(&self, socket_id: &str) {
        ...
        let rooms_to_leave: Vec<String> = {
            ...
            socket_rooms.get(socket_id).cloned().unwrap_or_default().into_iter().collect()
        };
    ...
    pub async fn join_room(&self, socket_id: &str, room_name: &str) -> Result<(), StreamError> {
        ...
        // Add to room
        {
            ...
            let room = rooms.entry(room_name.to_string()).or_insert_with(|| Room::new(room_name));
    ...
    pub async fn broadcast_to_room(&self, room_name: &str, msg: SocketMessage) -> Result<usize, StreamError> {
    ...
    pub async fn broadcast_to_room_except(
        ...
        for socket_id in members {
            if socket_id != except_id
                && self.send_to(&socket_id, msg.clone()).await.is_ok() {
                    sent += 1;
                }
        }
    ...
    pub async fn send_to_user(&self, user_id: &str, msg: SocketMessage) -> Result<usize, StreamError> {
    ...
    pub async fn is_user_online(&self, user_id: &str) -> bool {
        let user_sockets = self.user_sockets.read().await;
        user_sockets.get(user_id).map(|s| !s.is_empty()).unwrap_or(false)
    }
...
mod tests {
    ...
    fn create_authenticated_socket(id: &str, user_id: &str) -> (WebSocket, mpsc::Receiver<SocketMessage>) {
        let (tx, rx) = mpsc::channel(32);
        (WebSocket::authenticated(id.to_string(), user_id.to_string(), tx), rx)
    }
    ...
    async fn test_send_to() {
        ...
        hub.register(socket).await;
        hub.send_to("sock1", SocketMessage::text("hello")).await.unwrap();
    ...
    async fn test_broadcast_to_room() {
        ...
        let sent = hub.broadcast_to_room("room", SocketMessage::text("hello")).await.unwrap();
        assert_eq!(sent, 2);
    ...
    async fn test_broadcast_except() {
        ...
        let sent = hub.broadcast_to_room_except("room", SocketMessage::text("hello"), "sock1").await.unwrap();
        assert_eq!(sent, 1);
    ...
    async fn test_send_to_user() {
        ...
        let sent = hub.send_to_user("user1", SocketMessage::text("hello")).await.unwrap();
        assert_eq!(sent, 2);
    ...
    async fn test_send_to_nonexistent() {
        ...
        let result = hub.send_to("nonexistent", SocketMessage::text("hello")).await;
        assert!(result.is_err());
    }
