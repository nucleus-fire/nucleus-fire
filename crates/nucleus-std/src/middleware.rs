...
pub async fn request_logger(request: NucleusRequest, next: NucleusNext) -> NucleusResponse {
...
pub fn apply_cors_with_config(response: &mut NucleusResponse, config: &CorsConfig) {
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&config.allow_origins.join(", "))
...
pub fn rate_limit_middleware(
    ...
    NucleusRequest,
    NucleusNext,
) -> std::pin::Pin<Box<dyn std::future::Future<Output = NucleusResponse> + Send>>
       + Clone {
