...
impl axum::response::IntoResponse for SessionError {
    fn into_response(self) -> axum::response::Response {
        ...
        let (status, msg) = match &self {
            ...
            SessionError::InvalidCsrfToken => (StatusCode::FORBIDDEN, "Invalid CSRF token".to_string()),
            SessionError::Unauthenticated => (StatusCode::UNAUTHORIZED, "Authentication required".to_string()),
            SessionError::PermissionDenied(m) => (StatusCode::FORBIDDEN, m.clone()),
            SessionError::StorageError(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Session storage error".to_string()),
            SessionError::DeserializationError(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Session error".to_string()),
            SessionError::InvalidCookie => (StatusCode::BAD_REQUEST, "Invalid session cookie".to_string()),
        };
...
impl Session {
    ...
    pub fn get<T: DeserializeOwned>(&self, key: &str) -> Option<T> {
        self.data.data.get(key).and_then(|v| serde_json::from_value(v.clone()).ok())
    }
...
mod tests {
    ...
    fn test_session_string() {
        ...
        assert_eq!(session.get::<String>("string"), Some("hello world".to_string()));
    }
