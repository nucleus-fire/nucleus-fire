...
fn test_unclosed_tag() {
    ...
    if let Ok((rem, _nodes)) = result {
        assert!(!rem.trim().is_empty(), "Unclosed tag should leave remainder (failed to parse)");
    } else {
...
fn test_mismatched_tag() {
    ...
    if let Ok((rem, _)) = result {
        assert!(!rem.trim().is_empty(), "Mismatched tag should stop parsing and leave remainder");
    }
}
