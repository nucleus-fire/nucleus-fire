use crate::ast::{Element, Node, Component, Prop};
use nom::{
    branch::alt,
...
fn parse_element_node(input: &str) -> IResult<&str, Node> {
    ...
    let is_void = ["area", "base", "br", "col", "embed", "hr", "img", "input", "link", "meta", "param", "source", "track", "wbr"]
...
fn parse_for(input: &str) -> IResult<&str, Node> {
    ...
    let (input, (children, _)) = many_till(parse_node, 
        preceded(
            multispace0,
            tuple((
                tag("{%"),
...
fn parse_attr_value(input: &str) -> IResult<&str, String> {
    alt((
        ...
        map(delimited(tag("{{"), take_until("}}"), tag("}}")), |s: &str| format!("{{{}}}", s.trim()))
    ))(input)
}

fn map<I, O, E, F, O2>(parser: F, f: impl Fn(O) -> O2) -> impl FnMut(I) -> IResult<I, O2, E>
where F: FnMut(I) -> IResult<I, O, E>, E: nom::error::ParseError<I> {
...
fn parse_include(input: &str) -> IResult<&str, Node> {
    ...
    if !self_closing {
        ...
        Ok((input, Node::Include {
            path: attributes.iter().find(|(k, _)| k == "src").map(|(_, v)| v.clone()).unwrap_or_default(),
            attributes
        }))
    } else {
        Ok((input, Node::Include {
    ...
    for line in content.lines() {
        ...
        if let Some((name_part, rest)) = trimmed.split_once(':') {
            ...
            let (prop_type, default) = if let Some((type_part, default_part)) = rest.split_once('=') {
                (type_part.trim().to_string(), Some(default_part.trim().trim_matches('"').to_string()))
            } else {
...
fn parse_component(input: &str) -> IResult<&str, Node> {
    ...
    Ok((input, Node::Component(Component {
        name,
        props,
        children,
        styles,
...
fn parse_component_use(input: &str) -> IResult<&str, Node> {
    ...
    let children = if self_closing {
        ...
        if let Ok((_, nodes)) = parse_root(content) {
            return Ok((input, Node::ComponentUse {
                name: tag_name,
                props: attributes,
                children: nodes,
            }));
...
fn parse_model_node(input: &str) -> IResult<&str, Node> {
    ...
    Ok((input, Node::Model(crate::ast::Model {
        name,
        fields,
        methods,
        attributes,
...
mod tests {
    ...
    fn test_parse_model() {
        ...
        if let Node::Model(model) = &nodes[0] {
            ...
            assert_eq!(model.fields[0], ("username".to_string(), "String".to_string()));
        } else {
    ...
    fn test_parse_rich_model() {
        ...
        if let Node::Model(model) = &nodes[0] {
            ...
            assert_eq!(model.fields[0], ("username".to_string(), "String".to_string())); // Checked aliasing
...
mod parser_tests_v2 {
    ...
    fn test_for_loop() {
        ...
        if let Node::For { variable, iterable, children: _ } = node {
    ...
    fn test_nested() {
         ...
         if let Node::Element(el) = &nodes[0] {
             ...
             assert_eq!(el.children.len(), 1, "Expected 1 child (For loop) inside Div");
             
             if let Node::For { variable, iterable, children } = &el.children[0] {
...
mod parser_tests_v3 {
    ...
    fn test_parse_for_with_newlines() {
        ...
        match &nodes[0] {
            Node::For { variable, iterable, children: _ } => {
