...
impl Guardian {
    ...
    pub fn validate(&self, nodes: &[Node]) -> Result<(), String> {
        ...
        let has_spec = nodes.iter().any(|n| matches!(n, Node::Spec(_) | Node::Test(_)));
        ...
        for node in nodes {
            match node {
                Node::Element(el) => {
                    ...
                    if el.tag_name == "button" && el.children.is_empty() && !el.attributes.iter().any(|(k, _)| k == "aria-label") {
                         return Err("Accessibility Violation: <button> empty and missing 'aria-label'.".to_string());
                    }

                    // Rule 3: Anchors must have content
                    if el.tag_name == "a" && el.children.is_empty() && !el.attributes.iter().any(|(k, _)| k == "aria-label") {
                         return Err("Accessibility Violation: <a> link empty and missing 'aria-label'.".to_string());
                    }

                    // Rule 4: Inputs must have accessible label or identifier
                    if el.tag_name == "input" {
                        let attrs = &el.attributes;
                        let type_attr = attrs.iter().find(|(k, _)| k == "type").map(|(_, v)| v.as_str()).unwrap_or("text");
                        let ignored_types = ["hidden", "submit", "reset", "button", "image"];
                        
                        if !ignored_types.contains(&type_attr) {
                            let has_label = attrs.iter().any(|(k, _)| ["aria-label", "aria-labelledby", "title", "id", "placeholder"].contains(&k.as_str()));
                            if !has_label {
...
mod tests {
    ...
    fn test_guardian_passes_with_script_and_spec() {
        let nodes = vec![
            Node::Script { content: "fn main() {}".to_string(), attributes: vec![] },
    ...
    fn test_guardian_fails_img_no_alt() {
        let nodes = vec![
            Node::Element(crate::ast::Element {
