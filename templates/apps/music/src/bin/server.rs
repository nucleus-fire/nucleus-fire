...
async fn handle_navbar() -> impl IntoResponse {
    Html("No view found in src/views/components/navbar.ncl")
}
#[allow(non_snake_case, unreachable_code, unused_variables)]
async fn handle_LikeButton(
    headers: axum::http::HeaderMap,
    Query(params): Query<std::collections::HashMap<String, String>>,
) -> impl axum::response::IntoResponse {
    let mut html_body = String::new();
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/navbar", get(handle_navbar))
        .route("/LikeButton", get(handle_LikeButton))
