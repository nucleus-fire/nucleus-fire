use crate::models::{Album, Artist, Track, Video};
...
pub async fn scan_library(root_path: &str) {
    ...
    for entry in WalkDir::new(root_path).into_iter().filter_map(|e| e.ok()) {
        ...
        if ["mp4", "mkv", "avi", "mov", "webm"].contains(&ext.as_str()) {
            ...
            let existing = Video::query()
                .r#where("path", path_str.clone())
                .first::<Video>()
                .await
                .unwrap_or(None);
        ...
        if let Ok(tagged_file) = Probe::open(path)
            .map_err(|_| "err")
            .and_then(|p| p.read().map_err(|_| "err"))
        {
            let tag = tagged_file
                .primary_tag()
...
async fn get_or_create_artist(name: &str) -> i64 {
    // Check if exists
    if let Some(artist) = Artist::query()
        .r#where("name", name)
        .first::<Artist>()
        .await
        .unwrap_or(None)
