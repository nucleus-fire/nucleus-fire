...
async fn setup_test_db() {
    ...
    sqlx::query(schema).execute(pool).await.expect("Failed to migrate test db");
}
...
async fn test_api_endpoints() {
    ...
    // Seed data
    models::Artist::create().value("name", "API Artist").execute().await.ok();
    models::Video::create()
    ...
    let response = app.clone()
        .oneshot(Request::builder().uri("/api/artists").body(Body::empty()).unwrap())
        .await
        .unwrap();
    ...
    let response = app.clone()
        .oneshot(Request::builder().uri("/api/featured").body(Body::empty()).unwrap())
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    ...
    let response = app.clone()
        .oneshot(Request::builder().uri("/api/grouped").body(Body::empty()).unwrap())
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
