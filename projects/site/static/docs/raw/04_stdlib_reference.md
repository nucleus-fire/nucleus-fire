...
Health checks and readiness probes for k8s/cloud.

```rust
use nucleus_std::health::*;

// Define checks
health_check!("db", || async {
...
Request processing middleware system.

```rust
use nucleus_std::middleware::*;
...
```rust
use nucleus_std::pool_monitor::*;

// Metrics available at:
// GET /metrics
// - nucleus_db_connections_active
// - nucleus_db_connections_idle
```
...
Cron-like job scheduling for standard library.

```rust
use nucleus_std::scheduler::*;
...
File upload handling with S3/Local support.

```rust
use nucleus_std::upload::*;
...
async fn upload_handler(mut multipart: Multipart) {
    while let Some(field) = multipart.next_field().await? {
        ...
        // Save to configured storage provider
        let url = Uploader::new()
            .bucket("uploads")
            .key(&name)
            .upload(data)
            .await?;
